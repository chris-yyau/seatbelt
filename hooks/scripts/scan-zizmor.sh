...
[ "${SKIP_SEATBELT:-0}" = "1" ] && { seatbelt_log_skip "zizmor" "SKIP_SEATBELT"; exit 0; }
[ "${SKIP_ZIZMOR:-0}" = "1" ] && { seatbelt_log_skip "zizmor" "SKIP_ZIZMOR"; exit 0; }
...
if ! command -v zizmor &>/dev/null; then
    echo "SEATBELT DEGRADED: zizmor not installed — GitHub Actions scanning DISABLED (brew install zizmor | /seatbelt:setup)" >&2
    exit 0
fi
