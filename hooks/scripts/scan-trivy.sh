...
[ "${SKIP_SEATBELT:-0}" = "1" ] && { seatbelt_log_skip "trivy" "SKIP_SEATBELT"; exit 0; }
[ "${SKIP_TRIVY:-0}" = "1" ] && { seatbelt_log_skip "trivy" "SKIP_TRIVY"; exit 0; }
...
if ! source "$LIB_DIR/detect-commit.sh"; then
    echo "SEATBELT DEGRADED: trivy commit detection unavailable — trivy scan skipped" >&2
    exit 0
fi
